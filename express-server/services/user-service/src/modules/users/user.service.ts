...
import { hashPassword } from '@express-server/shared/src/lib/password';
import type { JwtPayload } from '@express-server/shared/src/lib/jwt';
import { prisma } from '@express-server/db-postgres';
import type { CreateUserInput, UpdateUserInput } from './user.schemas';
import { ROLE_HIERARCHY, type UserRole } from '@express-server/shared/src/types/user.roles';
...
export async function createUserByAdmin(input: CreateUserInput, actor: JwtPayload): Promise<UserResponse> {
...
export async function removeUser(userId: string, actor: JwtPayload): Promise<void> {
  const targetUser = await prisma.user.findUnique({ where: { id: userId } });
